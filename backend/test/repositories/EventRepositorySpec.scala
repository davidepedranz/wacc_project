...
final class EventRepositorySpec extends PlaySpec with GuiceOneAppPerTest with BeforeAndAfterEach {
  ...
  private def assertExists(date: Date) = {
    val result: List[Event] = Await.result(repository.readByDate(date), MAX_DURATION)
  ...
  override protected def beforeEach(): Unit = {
    Await.result(repository.initialize(), 5 seconds)
    repository._removeAllEvents()
  ...
  "#read" should {
    "return an empty list if the repository is empty" in {
      val result: List[Event] = Await.result(repository.readByDate(key1), MAX_DURATION)
      result mustBe empty
    }
    "return a list with an event after saving one event" in {
      saveEvent(TEST_EVENT_2)
      val result: List[Event] = Await.result(repository.readByDate(key2), MAX_DURATION)
